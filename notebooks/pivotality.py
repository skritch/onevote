...
def _():
    ...
    As a second answer, note that we just showed that the $P \propto \frac{1}{\sqrt{N}}$ rule describes both the probability of a single voter being pivotal *and* the probability that the election has a margin of $1$. Furthermore the probability of a margin of $k$, for any $k$, will be some expression like

    $$
    {N \choose \frac{N - 1}{2} - k} + \ldots + {N \choose \frac{N - 1}{2}} + \ldots + {N \choose \frac{N + 1}{2} + k}
    $$

    which *also* scales as $\frac{k}{\sqrt{N}}$. So it may be more satisfying to take this as a description of the probability that the *election* is very close, which, ex ante, is something like a probability that one's vote actually does something.

    It feels likely at this point that many "reasonable" measures of the weight of votes will reproduce the $\frac{1}{\sqrt{N}}$ scaling—this is, for example, the standard deviation of our $R$ variable as well:

    $$
    \text{SD}[R] = \sqrt{\frac{0.5(1-0.5)}{N}}
    $$

    I will add more ideas here as I come upon them.

    I still have a reservation along the lines of: what if you have additional information that the election *isn't* close? Logically, does this measure of pivotality just cease to mean anything? How do we handle this? [^notclose]

    [^notclose]: One answer might be that all information you could have is probabilistic, and the probability of a close election never goes to *zero* (considering unforeseeable rare events that might come up before the moment your vote is cast, unreliability of information, etc); there should still be some pivot probability which can be compared, even if it's low. We'll punt on taking this further for now. Here I think of calculating the pivot probability over a different distribution based on historical party shares or something. This would be viable as a metric!
    """)
    return
...
def _():
    ...
    $$
    \begin{align}
    P[\text{vote } x \text{ is pivotal nationally}] &= P[\text{vote } x \text{ is pivotal in state } s(x)] \times P[\text{state } s(x) \text{ is pivotal nationally}] \\
    ...
    But let us suppose that the individual states elector counts $e_s$ are small enough, relative to their total $E$, that the above may be treated by the Central Limit Theorem.
    ...
    In the last line I've taken an additional approximation to include $e_s$ in the sum in the denominator.
    ...
    Anyway—this is definitely something we can work with! Although we've taken so many approximations that it's hard to say how meaningful it is.


    We could simplify this a little more removing the restriction $s \ne  i$ from the sum; this shouldn't change the relative sizes of our values very much. Though the "true" value is not much harder to calculate. If we do this then the denominators for every state are equal to $\Vert \mathbf{e} \Vert = \sqrt{\sum_{s} {(e_s)}^2}$, where $\mathbf{e} = (e_1, \ldots, e_S)$ is the vector elector count. This will cancel out when we normalize the  measure.

    For the normalization constant we need:
    ...
    We will like to normalize this to add to $N$, as we've done with our other values. This will give us the value function we'll "**Pivotality Value**", or "PV":
    ...
    As we've done with the previous measures, let's visualize it.
